...
/**
 * Persist a new lead submission. Returns the inserted row id.
 */
export async function insertLead(lead: InsertLead): Promise<number> {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available; cannot save lead");
  }
...
/**
 * Fetch all leads, newest first. Intended for internal/admin use.
 */
